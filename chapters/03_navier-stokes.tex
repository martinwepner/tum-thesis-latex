...
Incompressible fluids are described by Navier Stokes equations:\\
...
Discretizing \ref{navier-stokes-pressure} in time and space gives for a discrete point in time $n$ the following equations:
...
\end{aligned}
\end{equation}
Now we have a system of linear equations in the form of 

\begin{equation} \label{pressure-equation-shortened}
	\mathbf{A}p = \frac{\rho \Delta x^2}{\Delta t}\nabla \cdot \vec{u}^n
\end{equation}
$\mathbf{A}$ is symmetric and each row represents one cell $(i, j)$ on the grid. 
...
Taking a closer look and with some rearrangements on equation \ref{pressure-equation} shows that it is an approximation of the Poisson problem $\frac{-\Delta t}{\rho} \nabla \cdot \nabla p = - \nabla \cdot \vec{u}$. Because of that I call $\mathbf{A}$ \textit{Laplace Matrix} in the following. 
...
\begin{itemize}
      ...
      \item For every neighbor of cell $(i,j)$ that is a solid cell increment $k_{diagonal}$ and set the corresponding column to 0.
